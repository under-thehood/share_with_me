...
const SendPage: React.FC<SendPageProps> = () => {
    ...
    return (
        <div className="container">
            ...
            {
                popUpArray.length > 1 ? <div className='reciever-popup' style={{ visibility: 'visible', top: getRandomIntString(100, 200), right: getRandomIntString(100, 200) }} onClick={() => {
                    invoke("ask_reciever_permission", { receiverInfo: { message: popUpArray[1] } });
                }}>{popUpArray[1]}</div> : <div className='reciever-avatar' style={{ visibility: 'hidden' }}></div>
            }
        </div >
    );
};
